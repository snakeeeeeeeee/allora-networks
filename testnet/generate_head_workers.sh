...
echo "generating allora account key for watch-prices"
mkdir -p ./watch-prices
$ALLORAD --home=./watch-prices keys add watch-prices --keyring-backend $keyringBackend > ./watch-prices/watch-prices.account_info 2>&1
watchPricesAlloraAccExport=$($ALLORAD --home=./watch-prices keys export watch-prices --unarmored-hex --unsafe --keyring-backend $keyringBackend)

watchPricesAlloraKeySecret="${CHAIN_ID}--watch-prices--allora-account"
echo "saving watch-prices allora key to $watchPricesAlloraKeySecret"
if aws secretsmanager describe-secret --secret-id $watchPricesAlloraKeySecret --region $AWS_REGION > /dev/null 2>&1 ; then
    aws secretsmanager put-secret-value \
        --secret-id $watchPricesAlloraKeySecret --region $AWS_REGION \
        --secret-string "$watchPricesAlloraAccExport"
else
    aws secretsmanager create-secret \
        --name $watchPricesAlloraKeySecret --region $AWS_REGION \
        --description "watch-prices allorad account's key export" \
        --secret-string "$watchPricesAlloraAccExport"
fi
